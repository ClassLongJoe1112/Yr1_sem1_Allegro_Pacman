...
int main(void) {
    ...
    game_event_queue = al_create_event_queue(); //括號入面冇嘢
    // 2) Check the pointer, call game_abort if the function failed.
    if (!game_event_queue)
        game_abort("failed to create event queue");
    // TODO: [Register display to event queue] (將display放入event queue)
...
void game_start_event_loop(void) {
    ...
    while (!done) {
        ...
        al_wait_for_event(game_event_queue, &event); //瘋狂run event queue入面嘅event
        // 2) If the event's type is ALLEGRO_EVENT_DISPLAY_CLOSE, set 'done' to true.
        if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE) { 
            // Event for clicking the window close button.
            game_log("Window close button clicked"); //click close button嘅event.type係ALLEGRO_EVENT_DISPLAY_CLOSE掛
            done = true;
        }
    }
}
