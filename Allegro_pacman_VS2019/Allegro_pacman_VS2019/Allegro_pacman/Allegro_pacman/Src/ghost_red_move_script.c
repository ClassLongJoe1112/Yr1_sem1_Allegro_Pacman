...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M) {
	...
	//for (Directions i = 1; i <= 4; i++)
		//if (ghost_movable(ghost, M, ghost->objData.preMove, true))  proba[cnt++] = i;
	...
	//random that ghost won't walk back and forth
	static Directions proba[4];
	int cnt = 0;
	for (Directions i = 1; i <= 4; i++) {
		//movable和facing的方向加起來不等於5, 才能走。 (如正在往UP走(1)，DOWN(4)是movable,就不設成可走的)
