...
extern ALLEGRO_SAMPLE* PACMAN_EATBEANSOUND;
...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	// That is to say, your pacman shouldn't penetrate 'wall' and 'room', where room is reserved for ghost to set up.
	...
	case RIGHT:
		...
		break;
	default:
		// for none UP, DOWN, LEFT, RIGHT direction you should return false.
		return false;
	}
...
Pacman* pacman_create() {
	...
	*/
	...
	pman->n_life = 2; //生命數系統用，這邊改的話，main scene中的小精靈圖示也會增加。

	//ALLEGRO_TIMER* al_create_timer(double speed_secs)
	pman->death_anim_counter = al_create_timer(1.0f/64); //1 tick/64 sec
...
void pacman_draw(Pacman* pman) {
	...
	*/
	...
	if (game_over) {
		...
		*/
		int64_t death_anim_counting= al_get_timer_count(pman->death_anim_counter);
		int image_num = 0;
		if (death_anim_counting < 10) image_num = 0;
		if (death_anim_counting >= 10 && death_anim_counting < 10 * 2) image_num = 16;
		if (death_anim_counting >= 10 * 2 && death_anim_counting < 10 * 3) image_num = 16 * 2;
		if (death_anim_counting >= 10 * 3 && death_anim_counting < 10 * 4) image_num = 16 * 3;
		if (death_anim_counting >= 10 * 4 && death_anim_counting < 10 * 5) image_num = 16 * 4;
		if (death_anim_counting >= 10 * 5 && death_anim_counting < 10 * 6) image_num = 16 * 5;
		if (death_anim_counting >= 10 * 6 && death_anim_counting < 10 * 7) image_num = 16 * 6;
		if (death_anim_counting >= 10 * 7 && death_anim_counting < 10 * 8) image_num = 16 * 7;
		if (death_anim_counting >= 10 * 8 && death_anim_counting < 10 * 9) image_num = 16 * 8;
		if (death_anim_counting >= 10 * 9 && death_anim_counting < 10 * 10) image_num = 16 * 9;
		if (death_anim_counting >= 10 * 10 && death_anim_counting < 10 * 11) image_num = 16 * 10;
		if (death_anim_counting >= 10 * 11 && death_anim_counting < 10 * 12) image_num = 16 * 11;
		
		al_draw_scaled_bitmap(pman->die_sprite, image_num, 0,
			16, 16,
			drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
			draw_region, draw_region, 0);
		
		/*
		game_log("image of dead pman\n");
		...
		*/
		...
		{
		case LEFT:
			if (pman->objData.moveCD % 4 == 0) {  //因為moveCD是-=2(speed)，所以不是被4整除，就是被2整除
				al_draw_scaled_bitmap(pman->move_sprite, 32, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0);
		...
		case DOWN:
			...
			if (pman->objData.moveCD % 4 == 0) {
				al_draw_scaled_bitmap(pman->move_sprite, 97, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0);
			}
			else {
				al_draw_scaled_bitmap(pman->move_sprite, 113, 0,
					16, 16,
					drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
					draw_region, draw_region, 0);
			}
...
void pacman_eatItem(Pacman* pacman, const char Item) {
	...
	case 'P':
		stop_bgm(PACMAN_MOVESOUND_ID);
		PACMAN_MOVESOUND_ID = play_audio(PACMAN_EATBEANSOUND, effect_volume);
		//game_log("you ate a powerbean");
		break;
	default:
		break;
	}
