...
static void ghost_pink_move_script_FREEDOM(Ghost* ghost,Pacman* pman, Map* M) {
	...
	Directions direc = 0;
	if (shortestDirection == UP) direc = UP;
	if (shortestDirection == DOWN) direc = DOWN;
	if (shortestDirection == LEFT) direc = LEFT;
	if (shortestDirection == RIGHT) direc = RIGHT;
	if (!ghost_movable(ghost, M, direc, true)) direc = DOWN;
	if (!ghost_movable(ghost, M, direc, true)) direc = UP;
	if (!ghost_movable(ghost, M, direc, true)) direc = RIGHT;
	if (!ghost_movable(ghost, M, direc, true)) direc = LEFT;
	ghost_NextMove(ghost, direc);
}
